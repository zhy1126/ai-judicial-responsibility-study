...
- 每案一份法官第一人称陈述，以音频配合逐步出现的文字呈现。当前尚未提供录音，先采用 15 秒逐步呈现文字；到达末尾并确认后继续。两份录音接入后按播放时间占总时长的比例揭示文字；暂停时文字停止，完整收听后才能继续。该算法按进度同步，不是逐字时间戳对齐；录音内容应与正文一致。
...
node tests/mobile-flow.test.cjs
node tests/narration-upgrade.test.cjs
node tests/screening-flow.test.cjs
node tests/narration-revision.test.cjs
node tests/responsibility-flow.test.cjs
```

可用 PLAYWRIGHT_PATH、PLAYWRIGHT_BROWSERS_PATH 和 STUDY_TEST_URL 指定测试环境。旧的 chatbot/dialogue 等浏览器脚本是历史版本回归，当前以 mobile-flow、screening-flow、responsibility-flow 为准。main 更新后 GitHub Actions 执行规则测试、构建并发布至原 GitHub Pages。构建不包含原始案例、测试、配音文字或内部说明。
...
背景题在正式页面、预览页面和需要重新核对的旧会话中均不预选；继续已有会话仍保留此前已提交的分组及答卷。本次界面协议为 `mobile-streaming-sliders-2026-09-17-v1`，逐步呈现版本为 `progressive-narration-2026-09-17-v1`。保留科学研究用途与撤回说明；删除参与者材料中的原始案号缺失、证据未提供等编辑备注，不补造案号或证据。
