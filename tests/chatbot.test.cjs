...
(async()=>{
 ...
 try{
  for(const mobile of [false,true]){
   ...
   const paragraphs=await page.evaluate(()=>[CONDITIONS[state.condition].intro,...CASES[state.caseType].narrative,CONDITIONS[state.condition].review]);
   ...
   assert.deepEqual(await page.locator('.typewriter-text').allTextContents(),paragraphs,'complete conversation keeps every original paragraph');
