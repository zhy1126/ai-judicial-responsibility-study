...
async function finish(p,audio=false){
 ...
 else {await p.locator('#narration-text').evaluate(e=>{e.scrollTop=e.scrollHeight;e.dispatchEvent(new Event('scroll'));});await p.clock.runFor(16000);}
...
(async()=>{const browser=await chromium.launch({headless:true});try{
 ...
 for(const role of roles)for(const condition of conditions)for(const firstCase of ['natural','statutory']){
  ...
  await fillBackground(p);await p.locator('#consent-checkbox').check();await p.locator('#start-study').click();
  ...
  for(let i=0;i<2;i++){
   ...
   await p.clock.runFor(15500);const text=await p.locator('#narration-text').innerText();if(texts[caseType])assert.equal(text,texts[caseType]);else texts[caseType]=text;
 ...
 await p.goto(`${base}/?view=participant&preview=1&role=public&condition=none&case=natural`);await fillBackground(p);await p.locator('#consent-checkbox').check();await p.locator('#start-study').click();await orient(p);
 for(let i=0;i<2;i++){await read(p);assert.equal(await p.locator('#audio-panel').isVisible(),true);await finish(p,true);assert.equal(await p.locator('#narration-text p').count(),i===0?5:6);if(i===0){await p.locator('#next-case').click();await orient(p);}}
 const record=await p.evaluate(k=>JSON.parse(localStorage.getItem(k))[0],recordsKey);assert.ok(record.responses.every(r=>r.presentation==='shared_audio_progressive_text'&&r.audio.completed));await c.close();
