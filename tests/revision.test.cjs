...
(async()=>{
 ...
 try{
  ...
  const oldAssignment=await oldPage.evaluate(k=>{
   const assignment=StudyCore.assignParticipant({legalIndustry:'no',practicingLawyer:'no',judgeCaseExperience:'no',legalDegree:'yes',litigationExperience:'yes'},{choose:x=>x[0],sessionId:'V2-KEEP'});
  ...
  for(const caseType of ['natural','statutory']){
   ...
   assert.equal(await audioPage.locator('#chat-messages').innerText(),'');
