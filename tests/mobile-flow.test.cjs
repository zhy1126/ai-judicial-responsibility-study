...
(async()=>{const b=await chromium.launch({headless:true});try{
 ...
 const records=await p.evaluate(()=>JSON.parse(localStorage.getItem(STORAGE_KEY)));assert.equal(records[0].responses.length,2);assert.equal(records[0].responses[0].ratings.fairness,6);assert.equal(records[0].responses[0].perceivedHarm,4);assert.equal(records[0].responses[0].involvement,4);assert.equal(records[0].responses[0].presentation,'shared_progressive_text');assert.deepEqual(errors,[]);
