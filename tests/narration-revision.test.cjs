...
(async()=>{const b=await chromium.launch({headless:true});try{
 ...
 await p.goto(`${base}/?view=participant&preview=1&role=public&condition=substantive&case=${caseType}`);await p.locator('#consent-checkbox').check();await p.locator('#start-study').click();
 const before=await p.evaluate(()=>{
  ...
  document.querySelector('#transcript-confirm').checked=true;document.querySelector('[name=fairness][value="6"]').checked=true;document.querySelector('#responsibility-score-judge').value='95';
 ...
 await p.reload();await p.locator('#screen-replay.active').waitFor();assert.deepEqual(await p.locator('#narration-text p').allTextContents(),expected[caseType]);
 assert.deepEqual(await p.evaluate(()=>state.assignment),before.assignment);assert.deepEqual(await p.evaluate(()=>state.reading),before.reading);assert.equal(await p.evaluate(()=>state.orientation[state.caseType].completed),true);assert.equal(await p.locator('[name=fairness][value="6"]').isChecked(),false);assert.equal(await p.locator('#responsibility-score-judge').inputValue(),'');
 assert.equal(await p.locator('#transcript-confirm').isDisabled(),true);await p.locator('#narration-text').evaluate(e=>{e.scrollTop=e.scrollHeight;e.dispatchEvent(new Event('scroll'));});await p.clock.runFor(31000);await p.locator('#transcript-confirm').check();await p.locator('#to-decision').click();await p.locator('#to-survey').click();await p.locator('#screen-survey.active').waitFor();assert.equal(await p.locator('#responsibility-score-list input').count(),4);
