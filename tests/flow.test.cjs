...
(async()=>{
 ...
 for(const role of ['lawyer','litigant','public'])for(const condition of ['none','procedural','substantive','decisional'])for(const caseType of ['natural','statutory']){
  ...
  assert.ok(decision.includes(caseType==='natural'?'十五年':'八个月'));
  assert.ok(decision.includes(caseType==='natural'?'三年':'九万元'));
  ...
  assert.equal(records.length,1);const response=records[0];assert.equal(response.role,role);assert.equal(response.condition,condition);assert.equal(response.caseType,caseType);assert.equal(response.preview,true);
 ...
 await context.close();
 console.log('Passed: 24 complete preview paths, mobile widths, no defaults, applicability, refresh, delete tombstone, real screening, manual fallback, legacy preservation and full CSV export.');
