...
const PRESENTATION_PROTOCOL = "case-role-broll-2026-09-16-v4";
...
function setupParticipant(){
  if(state.preview){
    ...
    for(const [name,value] of Object.entries({practicingLawyer:state.role==='lawyer'?'yes':'no',legalDegree:state.role==='lawyer'?'yes':'no',litigationExperience:state.role==='litigant'?'yes':'no'}))qs(`input[name="${name}"][value="${value}"]`).checked=true;
    qs('#screening-note').textContent='预览使用示例背景，按设计台选定的视角和条件呈现，不代表真实参与者。';
  }
  qsa('input[name="practicingLawyer"]').forEach(el=>el.addEventListener('change',updateScreening));updateScreening();
...
function startStudy(){
  ...
  const background=Object.fromEntries(['practicingLawyer','legalDegree','litigationExperience'].map(name=>[name,qs(`input[name="${name}"]:checked`)?.value||null]));
  try{
...
function snapshot(){if(state.deleted)return deletionMarker(state.assignment?.sessionId);return {epoch:storageEpoch,version:CORE.VERSION,assignment:state.assignment,session:state.session,retained:state.retained,consent:state.consent,orientation:state.orientation,step:state.step,ranking:state.ranking,rankingInitial:state.rankingInitial,reading:state.reading,activeDossierTab:state.activeDossierTab,formValues:state.formValues,replayExposureMs:exposure(),transcriptConfirmed:qs('#transcript-confirm').checked,response:state.response,deleted:state.deleted,audio:state.audio,replay:{...state.replay},speechMetadata:mergedSpeechMetadata()};}
function saveDraft(){
...
function updatePlaybackGate(){
  ...
  qs('#narration-reading-status').textContent=state.replay.mode==='audio'?'可边听边阅读下方文字，完整收听后继续。':state.replay.completed?'全文已阅读，请勾选确认后继续。':`请将正文阅读至末尾，并至少阅读 30 秒。${state.replay.textVisibleMs<NARRATION.MIN_TEXT_MS?'还需 '+Math.ceil((NARRATION.MIN_TEXT_MS-(state.replay.textVisibleMs||0))/1000)+' 秒。':''}`;
...
function updateRankingMode(){qs('#ranking-mode').value='rank';qs('#ranking-section').classList.remove('hidden');qs('#ranking-confirm').required=true;}
