...
const LABELS = {
  roles: {judge:'法官及法官助理视角',lawyer:'律师专业视角',litigant:'当事人视角',public:'公众视角'},
...
function setupParticipant(){
  if(state.preview){
    ...
    qs('#screening-note').textContent='研究者预览：请手动填写背景信息；后续按预览设定呈现。';
  }
  qsa('.screening-section input[type="radio"]').forEach(el=>{el.checked=false;el.autocomplete='off';});
...
function restoreDraft(){
  try{
    ...
    if(!state.preview&&!state.session.responses.length&&state.assignment.screeningVersion!==CORE.SCREENING_VERSION){
      ...
      qs('#intro-error').textContent='背景题已更新，请重新填写背景信息。原案件顺序和 AI 条件将保留。';
...
function advanceDossier(){
 const index=CORE.DOSSIER_TABS.indexOf(state.activeDossierTab);
 if(!state.reading[state.activeDossierTab]?.confirmed)return;
 if(index<CORE.DOSSIER_TABS.length-1){showDossierTab(CORE.DOSSIER_TABS[index+1]);qs('#screen-dossier').scrollIntoView({block:'start'});qs('#dossier-content').focus({preventScroll:true});}
 else beginReplay();
}
...
function renderTranscript(){
 ...
 const changed=state.replay.version!==NARRATION.VERSION||state.replay.mode!==mode||state.replay.presentationVersion!==StudyPlayback.VERSION;
 if(changed){
  const materialChanged=Boolean(state.replay.version||state.replay.mode);
  state.replay={mode,version:NARRATION.VERSION,presentationVersion:StudyPlayback.VERSION,completed:false,textReachedEnd:false,textVisibleMs:0,materialChanged};
...
function checkNarrationEnd(){
 ...
 if(state.replay.textRevealCompleted&&el.scrollTop+el.clientHeight>=el.scrollHeight-3)state.replay.textReachedEnd=true;
 updatePlaybackGate();
}
...
function updateAudioUI(){
  ...
  if(duration)state.audio.durationSeconds=duration;renderNarrationProgress();
...
function ratingControl(name,unsure=true){
 return `<input type="hidden" name="${name}" value=""><div class="range-shell"><input class="rating-range" data-range="${name}" type="range" min="1" max="7" step="1" value="4" aria-label="${name==='involvement'?'情境代入程度':RATINGS.find(x=>x[0]===name)?.[1]}" aria-describedby="feedback-${name}" aria-valuetext="尚未选择"><div class="range-ticks">${CORE.SCALE.map((label,i)=>`<button type="button" data-score="${i+1}" aria-label="${i+1} 分：${label}">${i+1}</button>`).join('')}</div><div class="range-anchors"><span>1 完全不同意</span><span>4 中立</span><span>7 完全同意</span></div></div>${unsure?'<label class="unsure-option"><input type="checkbox" data-unsure>无法判断</label>':''}<p id="feedback-${name}" class="rating-feedback" role="status">尚未选择，请拖动滑块或点击数字</p>`;
}
...
function updateRatingFeedback(){
 for(const field of qsa('[data-rating]')){
  const name=field.dataset.rating,value=qs(`input[name="${name}"]`,field).value,answered=/^[1-7]$/.test(value),range=qs('[data-range]',field);
  range.value=answered?value:'4';field.classList.toggle('has-rating',answered);range.setAttribute('aria-valuetext',answered?`${value} 分：${CORE.SCALE[Number(value)-1]}`:value==='unsure'?'无法判断':'尚未选择');
  ...
  qs('.rating-feedback',field).textContent=answered?`您选择了：${value} 分，${CORE.SCALE[Number(value)-1]}`:value==='unsure'?'您选择了：无法判断':'尚未选择，请拖动滑块或点击数字';
 }
}
